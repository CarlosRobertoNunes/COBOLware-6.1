       ...
       01  AREAS-DE-TRABALHO-1. COPY CWCASE.
           ...
           05 SAVE-WIDTH              PIC  X(003) VALUE SPACES.
           05 SAVE-FORM               PIC  X(001) VALUE SPACES.
           05 VAR                     PIC  X(030) VALUE SPACES.
           05 PROCESS-MSG             PIC  X(060) VALUE SPACES.
           05 PLIN                    PIC  9(002) VALUE 16.
           05 PCOL                    PIC  9(002) VALUE 12.
           05 BARRA-DIR               PIC  X(001) VALUE "\".
      ...
      *    05 PONTO                   PIC  9(002) VALUE 0.
           ...
           05 INTERNAL.
              10 CD-KEY               PIC S9(004) COMP-5.
              10 FILLER               PIC X(1998).
           05 PRN                     PIC  X(050) VALUE SPACES.
           05 LPT                     PIC  X(050) VALUE SPACES.
           05 TEMP                    PIC  X(050) VALUE SPACES.
           05 SPL000000               PIC  X(012) VALUE "R#000000.SPL".
           05 SPL999999               PIC  X(012) VALUE "R#999999.SPL".
           ...
           05 SPOOL-REMOTO             PIC  X(255) VALUE SPACE.
           05 SPOOL-CMD                PIC  X(255) VALUE SPACE.
           ...
           05 OLD-CWDIRS               PIC  X(255) VALUE SPACES.
           05 OLD-CWDIRS-IDX           PIC  X(255) VALUE SPACES.
           05 SAVE-CWDIRS              PIC  X(255) VALUE SPACES.
           05 VEZ-CWDIRS               PIC  9(001) VALUE 1.
           05 SALVA-CWCONF             PIC X(1956) VALUE SPACE.
           ...
           05 TITULO.
              10 BYTE-TITULO           PIC  X(001) OCCURS 174.
           05 SUB-TITULO.
              10 BYTE-SUB-TITULO       PIC  X(001) OCCURS 174.
           ...
           05 ER-CWSPLN.
              ...
              10 LB-CWSPLN             PIC  X(255) VALUE "CWSPLN".
           ...
           05 HORA.
              ...
              10 SS                    PIC  X(002).
              10 XX                    PIC  X(002).
GUI==>*    05 MSG                      PIC  X(030) VALUE "<<PRINTER>>".
GUI==>*    05 APAGA                    PIC  X(030) VALUE "<<<<<+>>>>>".
           ...
           05 file-offset              pic  x(008) comp-x.
           05 byte-count               pic  x(004) comp-x.
           05 flags                    pic  x(001) comp-x.
           05 GLB-WSTATION             PIC  X(017) VALUE SPACES.
      ...
      *COPY CWSPWS.
       ...
       01  PARAMETROS.
           ...
           05 CWIMPR-WITH-USER               PIC X(001).
           05 CWIMPR-RESERVED                PIC X(016).
       ...
       000-INICIO.
           ...
           IF  CHAMADAS = 0
               MOVE CWIMPR-WIDTH(1:) TO SAVE-WIDTH
               MOVE CWIMPR-FORM-TYPE TO SAVE-FORM
               IF  INSTANCIA = "CWIMPF"
                   DISPLAY 'XSEED-PRINTER' UPON ENVIRONMENT-NAME
                   MOVE    SPACES            TO GLB-WSTATION
                   ACCEPT  GLB-WSTATION    FROM ENVIRONMENT-VALUE
               END-IF
           END-IF
           ...
           IF   CWIMPR-SIZE-PAGE = 59 AND CHAMADAS = 0
                ...
                END-IF
                EVALUATE TRUE
                    WHEN CWIMPR-SIZE-132 OR CWIMPR-FORM-TYPE = '1'
                        DISPLAY "CWIMPR-SIZE-PAGE-132"
                                UPON ENVIRONMENT-NAME
                    WHEN CWIMPR-SIZE-080 OR CWIMPR-FORM-TYPE = '2'
                        DISPLAY "CWIMPR-SIZE-PAGE-080"
                                UPON ENVIRONMENT-NAME
                    WHEN CWIMPR-SIZE-220 OR CWIMPR-FORM-TYPE = '3'
                        DISPLAY "CWIMPR-SIZE-PAGE-220"
                                UPON ENVIRONMENT-NAME
                    WHEN OTHER
                         MOVE SPACES TO VAR
                         STRING "CWIMPR-SIZE-PAGE-" CWIMPR-SIZE-PAGE
                              DELIMITED BY SIZE INTO VAR
                         DISPLAY VAR UPON ENVIRONMENT-NAME
                END-EVALUATE
                PERFORM AJUSTA THRU FIM-AJUSTA
                IF  NUMERO NOT = 0
                    MOVE NUMERO TO CWIMPR-SIZE-PAGE
       ...
       CHECK-INCLUDE.
           ...
           ELSE
                ...
                IF   AUTOVIEW = 1
                OR   GLB-WSTATION = '<Spool>'
                     CALL "CWMEN9" USING CWDIRS-SPOOL
                     CANCEL "CWMEN9"
                ELSE
                     DISPLAY 'CWIMPR-SPOOL' UPON ENVIRONMENT-NAME
                     DISPLAY CWDIRS-SPOOL   UPON ENVIRONMENT-VALUE
                END-IF
       ...
       100-PROCESSAMENTO.
           ...
           OR   CWIMPR-REPORT      NOT = RELATORIO
                ...
                MOVE SPACES             TO PROCESS-MSG
                STRING "Gerando relat¢rio "
                       RELATORIO ' '        DELIMITED BY SIZE
                       SEM-PATH             DELIMITED BY "."
                       INTO PROCESS-MSG
                CALL 'CWPACK' USING PROCESS-MSG LENGTH PROCESS-MSG
...
GUI==>*    IF  NO-QUIET
GUI==>*    AND ((FOLHA NOT < RE-START)
GUI==>*    AND  (DIRECAO = 2))
GUI==>*         CALL "CBL_WRITE_SCR_CHARS" USING X"1602"
GUI==>*                                          MSG
GUI==>*                                          X"001E"
GUI==>*    END-IF
...
GUI==>*    END-IF
       ...
       170-ALINHA-TITULOS.
           ...
           INSPECT LINHA-3 (P: I) CONVERTING SPACE TO '-'.
       ...
       195-INSERE-PATH.
           ...
           IF   PATH-SPOOL NOT = SPACES
                ...
                PERFORM VARYING I FROM 1 BY 1
                        UNTIL LB-PRINTS(I:1)  = SPACES
                           OR I > LENGTH LB-PRINTS
                        IF   LB-PRINTS(I:1)  = "/"
                        AND  CWUNIX-OFF
                             MOVE "\" TO LB-PRINTS(I:1)
                        END-IF
                        IF  LB-PRINTS(I:1)  = "\"
                        AND CWUNIX-ON
                            MOVE "/" TO LB-PRINTS(I:1)
...
GUI==>*                                                    MSG
GUI==>*                                                    X"001E"
GUI==>*                   MOVE 1 TO DIRECAO
GUI==>*              ELSE
GUI==>*                   IF DIRECAO = 1
GUI==>*                      CALL "CBL_WRITE_SCR_CHARS" USING X"1602"
GUI==>*                                                       APAGA
GUI==>*                                                       X"001E"
GUI==>*                      MOVE 0 TO DIRECAO
GUI==>*                   END-IF
GUI==>*              END-IF
GUI==>*         END-IF
           ...
           ELSE
                ...
                if cwsplext = 'spl'
                   if byte-count < 254
                      ...
                      call "CWCODE" using "C" cwcode-tamanho
...
MD    *    AND  SPOOL-DEV NOT = "$"
       ...
       220-MAKE-PATH.
           ...
           PERFORM VARYING MP-I FROM LENGTH PATH-MD BY -1
                             UNTIL MP-I = 0
       ...
       230-ADD-TASK.
           ...
           IF   POSTER
           OR   IMPRESSORA
                MOVE    'spl'        TO CWDIRS-SPOOL (1:3)
                MOVE    'txt'        TO CWDIRS-SPOOL (10: 3)
           ELSE
                MOVE    CWSPLEXT     TO CWDIRS-SPOOL (10: 3)
      ...
      *                   CALL "SP2" USING SP2-GET-PANEL-DEF
      *                                    SP2-PANEL-DEF
      *                   MOVE SP2-PD-MSG-TEXT TO STATUS-DEF
      *                   MOVE SPACES TO LB-PRNTER-2
      *                   STRING SEM-PATH DELIMITED BY "."
      *                          INTO LB-PRNTER-2
     ...
     **                   CALL "CWRESE"
      ...
      *                                       SP2-PANEL-DEF
                     END-IF
GUI==>*              MOVE    SEM-PATH        TO MSG
GUI==>*              MOVE    SPACES          TO APAGA
                     IF   STATUS-REPORT = 2
       ...
       810-LER-CWCONF.
           ...
           DISPLAY 'CWIMPR-USER' UPON ENVIRONMENT-NAME
           ACCEPT OPERADOR       FROM ENVIRONMENT-VALUE
           MOVE 0                  TO AUTOVIEW
...
ks                        END-IF
      ...
      *         END-IF
           ELSE
                ...
                END-IF
                IF   FS-CWDIRS > "09"
                     MOVE "A lista de spool ser  reiniciada"
                       TO CWSEND-MSG
                     IF   FS-CWDIRS = "30" OR "35"
                          CONTINUE
                     ELSE
...
MD    *         AND  SPOOL-DEV NOT = "$"
                ...
                IF   E-MAIL NOT = SPACES
                     IF   CWIMPR-TITLE = SPACES
                          MOVE "Spool COBOLware 6.1" TO CWIMPR-TITLE
                     END-IF
                     MOVE SPACES TO EMAIL-TEXT
                     DISPLAY 'CWREPORTEXTMAIL' UPON ENVIRONMENT-NAME
                      ...
                      IF   POSTER
                           ...
                           IF   ORIENTACAO = "D"
                                ...
                                IF  CWPRINTFONT NOT = SPACES
                                    ...
                                    WRITE TEXTO-REG
                                END-IF
                                IF  GLB-WSTATION NOT = SPACE
                                    MOVE 'REPORT' TO CWDIRS-CODIGO
                                END-IF
       ...
       END-IMP.
           ...
           MOVE   "N"              TO FL-PRINTS
           MOVE "   0000/"         TO PAGE-TXT-POL
           MOVE SAVE-WIDTH         TO CWIMPR-WIDTH(1:)
           MOVE SAVE-FORM          TO CWIMPR-FORM-TYPE
