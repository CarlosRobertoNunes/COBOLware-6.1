      ...
      *----------------------------------------------------------------*
       ...
       01  W-INDICADORES.
           03 SINGULAR                 PIC X(20)    VALUE "REAL".
           03 PLURAL                   PIC X(20)    VALUE "REAIS".
           03 CWMONET                  PIC X(50)    VALUE SPACES.
           03 L-S                      PIC 9(02)    VALUE 0.
           03 L-P                      PIC 9(02)    VALUE 0.
           03 I                        PIC 9(02)    VALUE 0.
           03 MINUSCULAS PIC X(26) VALUE "abcdefghijklmnopqrstuvwxyz".
           03 MAIUSCULAS PIC X(26) VALUE "ABCDEFGHIJKLMNOPQRSTUVWXYZ".
      ...
      *
       ...
       01  W-TAB-NOME-11-A-19.
           ...
           03 FILLER                   PIC X(09) VALUE "dezessete".
      ...
      *                 PRIMEIRO NIVEL DO PROGRAMA                    *
       ...
       A01-PROCURA-DIGITO.
           ON 1
              DISPLAY "CWMONET" UPON ENVIRONMENT-NAME
              ACCEPT   CWMONET  FROM ENVIRONMENT-VALUE
              IF   CWMONET NOT = SPACES
                   MOVE SPACES TO SINGULAR PLURAL
                   STRING CWMONET DELIMITED BY ","
                          INTO SINGULAR
                   PERFORM VARYING I FROM 1 BY 1 UNTIL I > 50
                           IF  CWMONET (I: 1) = ","
                               ADD 1 TO I
                               MOVE CWMONET (I: ) TO PLURAL
                           END-IF
                   END-PERFORM
              END-IF
              IF SINGULAR = SPACES
                 MOVE "REAL" TO SINGULAR
              END-IF
              IF PLURAL = SPACES
                 MOVE "REAIS" TO PLURAL
              END-IF
              PERFORM VARYING L-S FROM 20 BY -1
                        UNTIL SINGULAR (L-S: 1) NOT = SPACE
                      CONTINUE
              END-PERFORM
              PERFORM VARYING L-P FROM 20 BY -1
                        UNTIL PLURAL (L-P: 1) NOT = SPACE
                      CONTINUE
              END-PERFORM
              INSPECT SINGULAR (1: L-S)
                      CONVERTING MAIUSCULAS TO MINUSCULAS
              INSPECT SINGULAR (1: 1)
                      CONVERTING MINUSCULAS TO MAIUSCULAS
              INSPECT PLURAL   (1: L-P)
                      CONVERTING MAIUSCULAS TO MINUSCULAS
              INSPECT PLURAL   (1: 1)
                      CONVERTING MINUSCULAS TO MAIUSCULAS.
       ...
       C03-NOME-UNIDADE.
           ...
           IF W-CENT(IND-C) = 0 AND W-CENT(IND-C + 1) = 0
                IF IND-2 = 1 AND IND-4 < 5
                     MOVE SINGULAR  TO W-NOME-AUX
                   ELSE
                     MOVE PLURAL   TO W-NOME-AUX
              ELSE
                MOVE SPACES TO W-NOME-AUX
                IF IND-2 = 1 AND IND-4 < 5
                     STRING SINGULAR (1: L-S) " e" DELIMITED BY SIZE
                         INTO W-NOME-AUX
                   ELSE
                     STRING PLURAL (1: L-P) " e" DELIMITED BY SIZE
                         INTO W-NOME-AUX.
           SET IND-3 TO 1.
           PERFORM D01-MOVER-LETRA
                   UNTIL W-LET-AUX(IND-3)     = SPACES
