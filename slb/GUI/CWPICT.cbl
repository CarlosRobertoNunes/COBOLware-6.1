      $Set CallFH"FHREDIR"
       ...
       01  AREAS-DE-TRABALHO. COPY CWCASE.
           ...
           05 DELETE-WORK           PIC  9(001) VALUE 0.
           05 TAMANHO               PIC  9(005) VALUE 0.
           05 SZ-BMPFILES           PIC  9(005) VALUE 58.
           ...
           05 ER-BMPWORK.
              ...
              10 LB-BMPWORK         PIC  X(255) VALUE SPACES.
              10 TEMP               PIC  X(255) VALUE SPACES.
              10 BMPWORK-LABEL                  VALUE "CW000000.BMP".
                 15                 PIC  X(002).
                 15 TASK            PIC  9(006).
                 15                 PIC  X(001).
                 15 EXTENSAO        PIC  X(003).
           05 SAVE-PARAMETROS-CWPICT PIC X(189).

       01  MAP VALUE LOW-VALUES.
           05 OCCURS 25.
              10 PIC-ID OCCURS 80 PIC S9(4) COMP-5.
       ...
       000-INICIO.
           ...
           IF   PARAMETROS-CWPICT = SAVE-PARAMETROS-CWPICT
           AND  CWPICT-DISPLAY
                GOBACK
           ELSE
                MOVE PARAMETROS-CWPICT TO SAVE-PARAMETROS-CWPICT
           END-IF

           IF  (PIC-ID (CWPICT-LINE CWPICT-COLUMN) NOT = 0)
           AND CWPICT-ERASE
               MOVE PIC-ID (CWPICT-LINE CWPICT-COLUMN) TO SP2-FD-ID
               CALL "CWSPID" USING SP2-FD-ID "FDelete"
               CALL SP2   USING SP2-DELETE-FIELD SP2-FIELD-DEF
               MOVE 0 TO PIC-ID (CWPICT-LINE CWPICT-COLUMN)
           END-IF

           MOVE SPACES TO IMAGE
                          LB-BMPWORK
           MOVE 0      TO DELETE-WORK
           ...
           IF  CWPICT-DISPLAY
               IF  IMAGE = SPACES
                   IF  (PIC-ID (CWPICT-LINE CWPICT-COLUMN) NOT = 0)
                       MOVE PIC-ID (CWPICT-LINE CWPICT-COLUMN)
                         TO SP2-FD-ID
                       CALL "CWSPID"
                            USING PIC-ID (CWPICT-LINE CWPICT-COLUMN)
                                    "FDelete"
                       CALL SP2   USING SP2-DELETE-FIELD SP2-FIELD-DEF
               ...
               ELSE
                   MOVE LOW-VALUES    TO SP2-FD-DATA
                                         SP2-FD-VAR-LENS
                   IF  (PIC-ID (CWPICT-LINE CWPICT-COLUMN) = 0)
                       CALL "CWSPID" USING PIC-ID
                                     (CWPICT-LINE CWPICT-COLUMN)
                                           "FInsert"
                   ELSE
                       MOVE PIC-ID (CWPICT-LINE CWPICT-COLUMN)
                         TO SP2-FD-ID
                   ...
                   END-IF
                   MOVE PIC-ID (CWPICT-LINE CWPICT-COLUMN) TO SP2-FD-ID
                   COMPUTE SP2-FD-ROW = CWPICT-LINE   - 1
                   COMPUTE SP2-FD-COL = CWPICT-COLUMN - 1
                   MOVE "o"          TO SP2-FD-OUTPUT
                   COMPUTE SP2-FD-HEIGHT = CWPICT-VERTICAL-LENGTH * 10
                   COMPUTE SP2-FD-WIDTH  = CWPICT-HORIZONTAL-LENGTH * 10
                   MOVE "i"    TO SP2-FD-CTRL-TYPE
                   MOVE SPACES TO SP2-FD-VAR-DATA
                   PERFORM VARYING I
                           FROM LENGTH OF IMAGE
                             BY -1
                                UNTIL I = 0
                                OR IMAGE (I:1) = '.'
                           CONTINUE
           ...
           IF   CWPICT-EXTRACT
                ...
                MOVE LB-BMPWORK TO CWPICT-IMAGE
           ELSE
                INITIALIZE PARAMETROS-CWPICT
           END-IF
